...
export default function FeedbackPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/feedback", {
      ...
      body: JSON.stringify({ itemid, from, to, message }),
    });
    const result = await res.json();
    setLoading(false);
    setSubmitMsg(result.message || "");
    if (result.success) {
      setTimeout(() => router.push(`/?itemId=${itemid}`), 1500);
    }
  }
    ...
    <div className="flex flex-col items-center justify-center min-h-screen p-4 bg-white">
      <div className="w-full max-w-md bg-gray-50 p-6 rounded shadow">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-3">
          ...
          <button type="submit" className="bg-blue-700 text-white p-2 rounded hover:bg-blue-900 transition" disabled={loading}>{loading ? "Submitting..." : "Submit Feedback"}</button>
        </form>
